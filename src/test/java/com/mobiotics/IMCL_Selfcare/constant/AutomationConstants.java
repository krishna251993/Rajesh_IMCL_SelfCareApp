package com.mobiotics.IMCL_Selfcare.constant;


public interface AutomationConstants
{
	public static  final String ACTUAL_PATH="\\actual\\";
	
	
	public static  final String CONFIG_PATH=System.getProperty("user.dir")+"\\Config\\";
	public static  final String CONFIG_FILE="config.properties";
	public static  final String DRIVER_PATH=System.getProperty("user.dir")+"\\Drivers\\";
	public static  final String CHROME_FILE="chromedriver.exe";
	public static  final String GECKO_FILE="geckodriver.exe";
	public static  final String CHROME_KEY="webdriver.chrome.driver";
	public static  final String GECKO_KEY="webdriver.gecko.driver";
	
	public static  final String EXPECTED_PATH="./expected/";
	public static  final String JARS_PATH="./jars/";
    public static  final String REPORT_PATH="./report/";
	public static  final String REPORT_FILE="report.html";
	public static  final String SNAP_PATH="./snap/";
	public static 	final String INPUT_PATH="./data/input.xlsx";
	
	
	public static  final String DOWNLOAD_PATH="\\Downloads\\";
	
	public static final String deviceName="My Phone";
	public static final String udid="4200b684b2b91400";
	public static final String platformName="Android";
	public static final String platFormVersion="6.0.1";
	public static final String appPackage="mobiotics.com.lcoselfcareimpl";
	public static final String appActivity=".newui.splash.SplashActivity";
	
	
}



